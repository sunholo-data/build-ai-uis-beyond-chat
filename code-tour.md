...
|---|---|---|---|
| 1 | [`backend/app.py`](https://github.com/sunholo-data/ai-protocol-platform/blob/main/backend/app.py) | 61 | The simplest ADK agent that exists |
| 2 | (Firestore seed) — `backend/db/local_fixture.py` `demo-researcher` block | ~30 | What a skill looks like as data |
| 3 | [`backend/adk/agent.py`](https://github.com/sunholo-data/ai-protocol-platform/blob/main/backend/adk/agent.py) | 488 | How a skill becomes a runnable LlmAgent |
| 4 | [`backend/skills/skill_processor.py`](https://github.com/sunholo-data/ai-protocol-platform/blob/main/backend/skills/skill_processor.py) | 331 | Where AG-UI events come from |
| 5 | [`frontend/src/hooks/useSkillAgent.ts`](https://github.com/sunholo-data/ai-protocol-platform/blob/main/frontend/src/hooks/useSkillAgent.ts) | 463 | How the frontend reads the event stream |
| 6 | [`backend/protocols/a2a.py`](https://github.com/sunholo-data/ai-protocol-platform/blob/main/backend/protocols/a2a.py) | 135 | Discovery in 135 lines |
| 7 | [`backend/protocols/mcp_proxy.py`](https://github.com/sunholo-data/ai-protocol-platform/blob/main/backend/protocols/mcp_proxy.py) | 386 | Frontend ↔ MCP server boundary |
...
- **When you deploy your fork:** [`docs/ops/deployment-models.md`](https://github.com/sunholo-data/ai-protocol-platform/blob/main/docs/ops/deployment-models.md) — picks single-service sidecar (default for most forks) vs paired sidecar+standalone (only if you run channel webhooks, external A2A crawlers, or external MCP callers) *before* first push, with a removal recipe for the unused `cloudbuild.yaml`. Workshop ran locally in `LOCAL_MODE=1` so this never came up, but it's the first decision on day one of operating your fork.
